...
class TaskService:
    ...
    async def create_task_with_priority(self, task_id: str, file_name: str, 
                                       file_path: str, file_size: int, 
                                       priority: str = "normal", options: dict = None):
        """创建带优先级的任务"""
        
        # 根据优先级设置超时和重试策略
        priority_config = {
            "high": {"timeout": 300, "max_retries": 3, "queue": "high_priority"},
            "normal": {"timeout": 600, "max_retries": 2, "queue": "default"},
            "low": {"timeout": 1800, "max_retries": 1, "queue": "low_priority"}
        }
        ...
        # 发送到相应优先级的队列
        await self._queue_task(task_id, config["queue"])
        
        return task
    
    async def get_task_metrics(self, task_id: str):
        """获取任务性能指标"""
        metrics = {
            "task_id": task_id,
            "timestamps": {},
        ...
        # 从Redis获取实时指标
        redis_key = f"task_metrics:{task_id}"
        redis_data = await self.redis_client.hgetall(redis_key)
        
        if redis_data:
            metrics["performance"] = {
                "memory_usage_mb": float(redis_data.get(b"memory_usage", 0)),
                "cpu_percent": float(redis_data.get(b"cpu_percent", 0)),
                "processing_rate": float(redis_data.get(b"processing_rate", 0))
            }
